...
public record SignInResponse(boolean success,
    ...
    public static SignInResponse success(User user,String message) {
        return new SignInResponse(true, message, user.getFName(), user.getLName(), user.getEmail());
    }

    public static SignInResponse failure(Boolean success,String message) {
        return new SignInResponse(success, message, null, null, null);
    }
}
